...
import cn.lanink.customitemapi.item.data.RenderOffsets;
...
public class CustomItemDefinition {
    ...
    public CustomItemDefinition(int id, CompoundTag nbt) {
        this.id = id;
        this.nbt = nbt;
    ...
    public boolean allowOffHand() {
        return this.nbt.getCompound("components")
                .getCompound("item_properties")
                .getBoolean("allow_off_hand", false);
    }

    public int getCreativeCategory() {
        return this.nbt.getCompound("components")
                .getCompound("item_properties")
                .getInt("creative_category");
    }

    public String getCreativeGroup() {
        return this.nbt.getCompound("components")
                .getCompound("item_properties")
                .getString("creative_group");
    }
    ...
    public static class SimpleBuilder {
        ...
        protected SimpleBuilder(IItemCustom item, ItemCreativeCategory creativeCategory) {
            this.id = item.getId();
            //定义材质
            this.nbt.getCompound("components")
                    .getCompound("item_properties")
                    .getCompound("minecraft:icon")
                    .putCompound("textures", new CompoundTag().putString("default", item.getTextureName()));
            //定义显示名
        ...
        public SimpleBuilder creativeGroup(String creativeGroup) {
            if (creativeGroup == null || creativeGroup.trim().isEmpty()) {
        ...
        public SimpleBuilder tag(String... tag) {
            ListTag<StringTag> list = this.nbt.getCompound("components").getList("item_tags", StringTag.class);
            if (list == null) {
        ...
        public CustomItemDefinition customBuild(Consumer<CompoundTag> nbt) {
            CustomItemDefinition def = this.build();
            nbt.accept(def.nbt);
            return def;
        }
        ...
        protected CustomItemDefinition calculateID() {
            return new CustomItemDefinition(this.id, nbt);
            //PM1E未实现字符串id，需要指定数字id
        }
    }
        ...
        public ToolBuilder addExtraBlockTags(@NonNull Map<Block, Integer> blockTags) {
            if (item.isPickaxe() || item.isShovel() || item.isHoe() || item.isAxe() || item.isShears()) {
                blockTags.forEach((block, speed) -> {
                    ...
                    this.blockTags.put(block.getName(), speed);
                });
            }
            return this;
        }
        ...
        public CustomItemDefinition build() {
            ...
            if (item.isShears() || item.isPickaxe() || item.isHoe() || item.isAxe() || item.isShovel()) {
                ...
                if (item.isPickaxe()) {
                    ...
                    for (Object name : gson.fromJson("[\"minecraft:soul_sand\",\"minecraft:soul_soil\",\"minecraft:dirt_with_roots\",\"minecraft:mycelium\",\"minecraft:podzol\",\"minecraft:dirt\",\"minecraft:farmland\",\"minecraft:sand\",\"minecraft:gravel\",\"minecraft:grass\",\"minecraft:grass_path\",\"minecraft:snow\",\"minecraft:mud\",\"minecraft:packed_mud\",\"minecraft:clay\"]\n", List.class)) {
                        this.blockTags.put((String) name, speed);
                    }
                } else if (item.isHoe()) {
                    this.nbt.getCompound("components").getCompound("item_properties")
                    ...
                    for (Object name : gson.fromJson("[\"minecraft:nether_wart_block\",\"minecraft:hay_block\",\"minecraft:target\",\"minecraft:shroomlight\",\"minecraft:leaves\",\"minecraft:leaves2\",\"minecraft:azalea_leaves_flowered\",\"minecraft:azalea_leaves\",\"minecraft:warped_wart_block\"]\n", List.class)) {
                        this.blockTags.put((String) name, speed);
                    }
                }
                //添加一些额外的挖掘tags
                for (Map.Entry<String, Integer> k : this.blockTags.entrySet()) {
